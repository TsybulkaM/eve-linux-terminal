...
// This library requires base support functions for SPI, delays, hardware pin          |
// control and more                                                                    |
// =====================================================================================
...
// Just in case this is for Arduino - EVE wants a DISPLAY() macro and Arduino already defines
// DISPLAY for something else that we will not be using, so we can kill the Arduino define.
#if defined(DISPLAY)
#undef DISPLAY
#endif
...
#define OPT_LEFTX 0UL
#define VERTEX_TRANSLATE_X(x) ((43UL << 24) | ((x)&65535UL))
#define VERTEX_TRANSLATE_Y(y) ((44UL << 24) | ((y)&65535UL))
...
#define RAM_G 0x0
#define RAM_G_WORKING                                                                             \
  0x0FF000 // This address may be used as the start of a 4K block to be used for copying data
...
// Discussion: Defining this way leads to an additional add operation in code that can be avoided
// by defining these addresses as 32 bit values, but this is easily paid for in clarity and
// coorelation to documentation. Further, you can add defines together in code and allow the
// precompiler to do the add operation (as here).
...
// Resistive Touch Engine Registers - FT81x Series Programmers Guide Section 3.3.3 - Document
// confused Addresses defined as offsets from the base address called RAM_REG and located at
// 0x302000
...
// Coprocessor Engine Registers - FT81x Series Programmers Guide Section 3.4
// Addresses defined as offsets from the base address called RAM_REG and located at 0x302000
...
// Addresses assumed to be defined as offsets from the base address called RAM_REG and located at
// 0x302000
...
// Miscellaneous Registers - FT81x Series Programmers Guide Section 3.6 - Document inspecific about
// base address Addresses assumed to be defined as offsets from the base address called RAM_REG and
// located at 0x302000
...
#define CLEAR(c, s, t)                                                                            \
  ((38UL << 24) | (((c)&1UL) << 2) | (((s)&1UL) << 1) |                                           \
   (((t)&1UL) << 0)) // CLEAR - FT-PG Section 4.21
#define CLEAR_COLOR_RGB(red, green, blue)                                                         \
  ((2UL << 24) | (((red)&255UL) << 16) | (((green)&255UL) << 8) |                                 \
   (((blue)&255UL) << 0)) // CLEAR_COLOR_RGB - FT-PG Section 4.23
...
#define VERTEX2F(x, y)                                                                            \
  ((1UL << 30) | (((x)&32767UL) << 15) | (((y)&32767UL) << 0)) // VERTEX2F - FT-PG Section 4.47
...
#define BITMAP_HANDLE(handle)                                                                     \
  ((5UL << 24) | (((handle)&31UL) << 0)) // BITMAP_HANDLE - FT-PG Section 4.06
#define BITMAP_SOURCE(addr)                                                                       \
  ((1UL << 24) | (((addr)&1048575UL) << 0)) // BITMAP_SOURCE - FT-PG Section 4.11
#define BITMAP_LAYOUT(format, linestride, height)                                                 \
  ((7UL << 24) | (((format)&31UL) << 19) | (((linestride)&1023UL) << 9) |                         \
   (((height)&511UL) << 0)) // BITMAP_LAYOUT - FT-PG Section 4.07
#define BITMAP_LAYOUT2(linestride, height)                                                        \
  ((28UL << 24) | (((linestride >> 10) & 3) << 2) | ((height >> 9) & 3))
#define BITMAP_SIZE(filter, wrapx, wrapy, width, height)                                          \
  ((8UL << 24) | (((filter)&1UL) << 20) | (((wrapx)&1UL) << 19) | (((wrapy)&1UL) << 18) |         \
   (((width)&511UL) << 9) | (((height)&511UL) << 0)) // BITMAP_SIZE - FT-PG Section 4.09
#define TAG(s) ((3UL << 24) | (((s)&255UL) << 0))    // TAG - FT-PG Section 4.43
#define POINT_SIZE(sighs)                                                                         \
  ((13UL << 24) | (((sighs)&8191UL) << 0)) // POINT_SIZE - FT-PG Section 4.36
#define LINE_WIDTH(width)                                                                         \
  ((14UL << 24) | (((width)&8191UL) << 0)) // POINT_SIZE - FT-PG Section 4.36
#define BEGIN(PrimitiveTypeRef)                                                                   \
  ((31UL << 24) | (((PrimitiveTypeRef)&15UL) << 0)) // BEGIN - FT-PG Section 4.05
#define END() ((33UL << 24))                        // END - FT-PG Section 4.30
#define DISPLAY() ((0UL << 24))
#define SCISSOR_SIZE(w, h) ((28UL << 24) | (((w)&4095UL) << 12) | (((h)&4095)))
#define SCISSOR_XY(x, y) ((27UL << 24) | (((x)&2047UL) << 11) | (((y)&2047)))
#define SCISSOR_SIZE(w, h) ((28UL << 24) | (((w)&4095UL) << 12) | (((h)&4095)))
#define SCISSOR_XY(x, y) ((27UL << 24) | (((x)&2047UL) << 11) | (((y)&2047)))
...
#define MAKE_COLOR(r, g, b) ((r << 16) | (g << 8) | (b))
  ...
  void EVE_EXPORT rdN(uint32_t address, uint8_t *buffer, uint32_t size);
  ...
  void EVE_EXPORT Cmd_Slider(uint16_t x,
                             uint16_t y,
                             uint16_t w,
                             uint16_t h,
                             uint16_t options,
  ...
  void EVE_EXPORT
  Cmd_Number(uint16_t x, uint16_t y, uint16_t font, uint16_t options, uint32_t num);
  void EVE_EXPORT
  Cmd_Gradient(uint16_t x0, uint16_t y0, uint32_t rgb0, uint16_t x1, uint16_t y1, uint32_t rgb1);
  void EVE_EXPORT Cmd_Button(uint16_t x,
                             uint16_t y,
                             uint16_t w,
                             uint16_t h,
                             uint16_t font,
  ...
  void EVE_EXPORT Cmd_Text_Codepoints(uint16_t x, uint16_t y, uint16_t font, uint16_t options, const char* utf8_str);
  ...
  void EVE_EXPORT Calibrate_Manual(uint16_t Width,
                                   uint16_t Height,
                                   uint16_t V_Offset,
                                   uint16_t H_Offset);
  void EVE_EXPORT Cmd_SetFont2(uint32_t handle, uint32_t addr, uint32_t firstChar);
